...
class owner {
   public:
    owner(ownerType type) { type_ = type; }

    virtual ~owner() {}

    ownerType get_type() const { return type_; }
...
class connection : public std::enable_shared_from_this<connection<T>> {
   public:
    connection(owner<T>& parent, asio::io_context& asioContext, asio::ip::tcp::socket socket,
    ...
    void send(const message<T>& msg) {
        asio::post(asioContext_, [this, msg]() {
   ...
   private:
    bool validate_header(message<T>& msg) { return owner_.validate_header(msg); }
                         ...
                         [this](std::error_code ec, std::size_t length) {
                             if (!ec) {
                                 ...
                                 if (validate_header(tempMessage_)) {
                                     ...
                                     std::cout << "[" << id_ << "]"
                                               << " Client is not valid, disconnect\n";
                                     disconnect();
                                 }
                             } else {
                                 std::cout << "[" << id_ << "]"
